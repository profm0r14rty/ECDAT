# Project Conventions (read-only, loaded every session)

This is the durable, per-session convention document for the ECDAT repo. It is
loaded read-only by Aider every session (see the `read:` entry in
`.aider.conf.yml`). Keep it technical and current — anything about how a
specific AI tool is invoked during development belongs in external workflow
notes, not here.

## Environment
- Omarchy Linux, terminal-native — no GUI tools, no Mac/Windows paths or line endings.
- Stack: Python 3.12 scanner engine (`ecdat_core/`, runtime dependency `pydantic>=2` only), FastAPI backend (`backend/`), React + Vite + TypeScript + Tailwind v4 frontend (`frontend/`), Postgres, Redis, Docker.
- A project virtualenv already exists at `test_virt_env/` (gitignored), package installed via `pip install -e ".[dev]"`. Reuse it — don't create a second venv.

## Editing rules
- Never touch files outside the ones explicitly added to chat for this phase.
- Match existing naming/formatting in the file you're editing — don't reformat untouched code.
- No new dependencies without flagging it in your commit message.
- Type-hinted Python, Pydantic v2 models, pytest for all new logic, small single-purpose functions, docstrings on every public function.
- Never silently change what an existing `signatures.json` entry matches or how it's classified — if a fix changes an existing signature's behavior, say so explicitly so downstream assumptions (tests, docs, frontend) get updated too.
- When a real ambiguity or contradiction turns up between these instructions and what a required test demands, resolve it in favor of technical correctness as documented here and state the deviation explicitly — don't silently pick one interpretation.

## Git
- One logical change per commit. Aider auto-commits — don't ask before committing, do ask before anything destructive (deleting files, force-pushing, altering migrations).
- Commit messages follow `"Phase N: <description>"` (or `"Fix Phase N: <description>"`).

## Testing / build
- Backend/scanner tests: `test_virt_env/bin/python -m pytest` from the repo root.
- Network-gated tests (git-clone tests) are marked `network` and deselected by default. Run them explicitly with `-m network` only when network access is confirmed available; don't assume it is.
- Frontend: from `frontend/`, `npm run build` (`tsc -b && vite build`). It must stay free of TypeScript errors — run `tsc -b` via the build before claiming a phase done.
...
- `backend/` — FastAPI app. Wraps `ecdat_core`; never re-implements scanning logic in the API layer.
- `frontend/` — React + Vite + TS + Tailwind dashboard.

## Output standard — CycloneDX 1.6 CBOM
Real CycloneDX 1.6 CBOM JSON. Component `type: "cryptographic-asset"`. `cryptoProperties.assetType` in `[algorithm, certificate, protocol, related-crypto-material]`. Never invent custom top-level fields — anything CycloneDX 1.6 has no slot for goes under `properties` with an `ecdat:` prefix (existing: `ecdat:riskLevel`, `ecdat:quantumVulnerable`, `ecdat:classicallyBroken`, `ecdat:confidence`, `ecdat:recommendedAlgorithm`, `ecdat:fipsReference`). The exported CBOM is validated against the official CycloneDX 1.6 JSON Schema (vendored at `ecdat_core/tests/fixtures/cyclonedx/`) in `ecdat_core/tests/test_cbom_schema.py`.

**Changelog — Phase 48 (breaking):** boolean-valued `ecdat:` properties (`ecdat:quantumVulnerable`, `ecdat:classicallyBroken`) serialize as lowercase `"true"`/`"false"` string-booleans, not Python-style `"True"`/`"False"`, matching conventional JSON-adjacent expectations. Anything already parsing the old casing must be updated. Same phase: `cryptoProperties.algorithmProperties.primitive` now uses only official 1.6 enum values (`key-agree` not `key-agreement`, `kem` for ML-KEM, `pke` for RSA, `unknown` instead of `null` for unclassifiable families) and `parameterSetIdentifier` is omitted rather than `null` when no key size is known — the official schema rejects `null` for both fields.
...
**API access control — optional single-tier Bearer API-key gate:**
- Minimum viable gate for non-demo deployments. **OFF by default** (`REQUIRE_API_KEY` unset/false) — every endpoint behaves exactly as before, so the live Render demo and DEMO.md's walkthrough are untouched. Full multi-tenant auth (users, orgs, roles) is deliberately separate, later work; this is the gate, not the product feature.
- Controlled by `REQUIRE_API_KEY` (env, default `false`) and `API_KEYS` (env, comma-separated). Both are read **per request** in `backend/app/security.py`'s `require_api_key` dependency — flipping the gate needs no restart and tests toggle it per-test on the same app instance. Generate real keys out-of-band (`python -c "import secrets; print(secrets.token_urlsafe(32))"`); never commit real values — `.env.example` keeps placeholders.
- When `true`, **every** `/api/scans/*` endpoint — writes AND reads — requires `Authorization: Bearer <key>`. Documented decision (gating reads too): the motivating leak is "anyone with the URL reads everyone's scan history / artefacts / CBOM exports", and a write-only gate fixes half of it. A `401` with `WWW-Authenticate: Bearer` answers missing/invalid keys; comparison is constant-time via `secrets.compare_digest`. `GET /health` stays open for Render health checks, UptimeRobot warm-keeping, and DEMO.md's warm-up step.
- The dependency sits at the router level (`dependencies=[Depends(require_api_key)]` on the `APIRouter` in `backend/app/routers/scans.py`), so it runs before every handler — an unauthenticated caller gets 401, not 404, and cannot even probe scan existence.
- **Fail-closed config error**: `REQUIRE_API_KEY=true` with `API_KEYS` empty/unset → `500` with a clear message. A misconfigured "protected" API must never silently become an open one.
- **Bundled dashboard caveat**: `frontend/src/api/client.ts` sends no auth header by design (the dashboard is a public-demo artifact), so flipping the gate on also blocks the dashboard's `/api/scans` calls. A non-demo deployment uses API-keyed clients or terminates the key at a same-origin proxy; key entry in the UI is out of scope for this gate.

**Rate limiting — scan creation only:**
- `POST /api/scans` is throttled per client via slowapi (`backend/app/rate_limit.py`). Identity is API-key-based when `REQUIRE_API_KEY=true` and a valid key rides along (`apikey:<sha256(key)>` — prevents combining many keys into one shared IP pot), IP-based otherwise (deliberately `get_remote_address`, NOT `X-Forwarded-For`, so the bucket can't be spoofed — consequence: a single-worker deployment behind a proxy buckets all clients together until the key gate is on).
- `SCAN_CREATE_RATE_LIMIT` (env, default `10/hour`, read per request like the API-key gate) in limits' `"N/unit"` format. `RATE_LIMIT_STORAGE_URI` (env, default `memory://`; `redis://...` only for multi-worker deployments sharing one bucket) is read once at import. **Fail-closed config error**: an invalid limit spec raises at import (500) — slowapi swallows `ValueError` from dynamic-limit callables and would otherwise silently stop limiting (fail-open); `rate_limit.py` therefore raises `RuntimeError` instead.
- Exceeded limit → `429` with `{"detail": "Rate limit exceeded: <spec>"}` and a `Retry-After` header (seconds; computed from the storage window's reset time). The limit is checked inside the endpoint wrapper, so 422s (schema validation) and 401s (router-level auth gate) run first and never consume the bucket.
- **Schema-layer validation tightening (Phase 47)**: `CreateScanRequest.target` capped at 2048 chars; git_url targets must be URL-shaped (`scheme` + `netloc`, via a `model_validator`) or they 422 before reaching ingestion (previously some garbage — e.g. `not-a-url`, `file:///etc/passwd` — reached `validate_git_url` and 400'd; shape-valid wrong schemes still 400 "Only https://" there, https-policy stays single-source in `ecdat_core/ingestion.py`); `ArtefactOverrideRequest.shelf_life_years`/`migration_time_years` bounded `0..200` (422).
- Test isolation: conftest's autouse fixture resets the process-wide limiter storage before every test, so bucket consumption never leaks across tests.
...
## App layer (`ecdat/`) — CLI + TUI + PyPI (added Phase 58)
- **Layering:** `ecdat_core` (engine) ← `ecdat` (CLI + TUI app). The engine NEVER imports `ecdat`, `rich`, `textual` or anything UI-related. The app layer uses only the engine's public API (`run_scan`, `export_cbom`, `export_summary`, `models`, `signature_loader`, `risk_engine`, `progress`). Never import from `backend/`.
- **Distribution:** PyPI name `ecdat`; one wheel ships both import packages `ecdat` and `ecdat_core`. Console script `ecdat = "ecdat.cli:main"`; `python -m ecdat` must behave identically. `python -m ecdat_core.cli` (legacy argparse CLI) stays untouched and working.
- **CLI stack:** stdlib `argparse` + Rich. Do NOT add Typer/Click. Every module in `ecdat/cli/commands/` exposes `register(subparsers)` and `run(args) -> int`. `ecdat.cli` must never import `textual` (the TUI is imported lazily inside its command).
- **Screen data contract:** CLI renderers, exporters and the TUI consume ONLY `ecdat.services.viewmodel.ScanVM`/`FindingVM`; engine models are touched in exactly one place (`build_scan_vm`). Never duplicate the detection→risk→recommendation join.
- **Untrusted content (security rule):** file paths, matched snippets, algorithm/library names, repo names and URLs from scanned repos are ATTACKER-CONTROLLED. Never put them into a Rich/Textual markup string or an f-string that is later parsed as markup. Use `rich.text.Text(value)` (or `rich.markup.escape`); HTML → `html.escape(value, quote=True)`; Markdown tables → escape `|`, backticks, newlines; JSON/SARIF → `json.dumps`. Every new renderer needs a test with hostile names like `[bold red]x[/].py` and one containing `\x1b[31m`.
- **Output discipline:** payload formats (`--format json|cbom|summary|markdown|html|sarif`) write ONLY the payload to stdout (via `sys.stdout.write`); progress/status/errors go to stderr. Only `ecdat/ui/console.py` may create a Rich Console; no bare `print()` in app code. Colors come from `ecdat/ui/theme.py` tokens, never hardcoded.
- **Exit codes:** 0 ok · 1 findings at/above `--fail-on` · 2 usage/validation error · 3 scan/runtime/unexpected error · 130 interrupted. No tracebacks for expected errors (`ScanError`); unexpected errors go through `ecdat/services/crashlog.py`.
- **Trust boundary:** CLI/TUI scan local paths with `sandboxed=False` (the user is the trust boundary); the HTTP API keeps the workspace sandbox. Git-URL SSRF validation stays ON for CLI/TUI. No telemetry, no update checks, no network use except `git clone` for URL scans — this is tested and documented; keep it true.
- **Python/Windows-safe:** write Python 3.10-compatible code (no `typing.Self`, `datetime.UTC`, `tomllib`, `StrEnum`, `ExceptionGroup`/`except*`, PEP 695 generics, PEP 701 nested-quote f-strings). pathlib everywhere; `encoding="utf-8"` on every text open; `FindingVM.file_path` uses forward slashes; no POSIX-only APIs.
- **User data:** everything lives under `ECDAT_HOME` (or the platform data dir) via `ecdat/services/paths.py`; app-layer tests set `ECDAT_HOME` to a tmp dir (autouse fixture in `ecdat/tests/conftest.py`) and `ECDAT_ANIM=0` — a test must never touch the real home or animate.
- **Textual:** only documented public APIs; pinned major in pyproject; scans run in worker threads (`@work(thread=True)`), UI updated via `call_from_thread`, never block the event loop; UI tests use `App.run_test()` (headless) with `@pytest.mark.asyncio`; untrusted strings enter widgets only as `rich.text.Text`; every animation goes through `ecdat/ui/motion.py`.
- **Tests:** app-layer tests live in `ecdat/tests/` with UNIQUE basenames `test_app_*.py` (the repo has flat test modules elsewhere); run `test_virt_env/bin/python -m pytest ecdat_core ecdat -q`. Markers: `network` (deselected by default), `packaging` (builds the wheel; slow).
- **Do-not-touch list (unless the phase names them):** `backend/**`, `frontend/**`, `render.yaml`, `docker-compose.yml`, `verification/**`, `ecdat_core/signatures.json`, `ecdat_core/tests/fixtures/**`, existing `ecdat_core/tests/test_*.py`, `DEMO.md`, `AGENTS.md`, `progress.md`.
